...
class Main(Login,Admin,User):
    ...
    def __init__(self):
        ...
        self.loginw.state('withdraw')  # sisselogimisaknad väljuvad
    ...
    def __Main_del__(self):
        if messagebox.askyesno("Lõpetab", " väljumine Inventuurist?") == True:
            self.loginw.quit()
            self.mainw.quit()
            exit(0)
        else:
    ...
    def buildmain(self):
        ...
        self.storelable=Label(self.topframe,text=self.store_name + "' müügi- ja laoseisusüsteem",bg="#4267b2",anchor="center")
    ...
    # ESTE JA KASUTAJA NUPPUDE MUUTMISE MEETODID
    def change_user(self):
        if messagebox.askyesno("Viga!", "Kas soovite kasutajat vahetada?") == True:
            self.base.commit()
            self.mainw.destroy()
            self.loginw.destroy()
            self.__init__()
